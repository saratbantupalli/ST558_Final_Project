...
function(input, output, session) {
  ...
  # # # For numerical summaries, create a reactive variable so that the 
  # data is based on user input
  ...
  # # # # For graphical summaries(Securities Trend Plot and Securities Histogram), 
  ...
  # # For graphical summaries(Total Debt and Treasury Security Interest Rates plot), 
  ...
  # # For graphical summaries(Correlation Plot), 
  # create a reactive data of combined_data 
  ...
  output$app_purpose <- renderUI(
        ...
        different regression models.","</br>" , "In the data exploration tab, the user 
        can choose see", "<em>", "trends in average interest rates for 7 different
        marketable securities", "</em>", "including Treasury Bills, Treasury Notes, 
        Treasury Bonds, Treasury Inflation-Protected Securities (TIPS), 
        Treasury Floating Rate Notes (FRN), 
        Federal Financing Bank, and Total Marketable. In addition, the user can
        see", "<em>", "distribution of various US Treasury Securities, trends in National
        Public Debt, and Correlation of US Treasury Securities with National Debt", "</em>", 
        "</br>", "</br>",
    ...
    graphical_plot <- reactive({
      ...
      else if(input$type_of_plot == "Total Debt and Treasury Security Interest Rates"){
        g3 <- combined_subset_data() %>% 
          ggplot(aes(x = values, 
                     y = `Total Public Debt Outstanding`)) + 
          geom_point(aes(color = security), alpha = 0.5) + 
